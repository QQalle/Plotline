...
public struct PlotDomain: Sendable, Hashable {
  ...
  public func padded(by fraction: Double, includeZero: Bool = false) -> PlotDomain {
    let safeFraction = fraction.isFinite ? max(0, fraction) : 0
    let expanded = expandedIfConstant()
    let padding = expanded.span * safeFraction
    var lower = expanded.lowerBound - padding
    var upper = expanded.upperBound + padding
...
public struct PlotScale: Sendable, Hashable {
  ...
  public init(
    domain: PlotDomain, range: ClosedRange<Double>, direction: PlotAxisDirection = .normal
  ) {
  ...
  public func position(for value: Double, clamped: Bool) -> Double {
    position(for: clamped ? domain.clamped(value) : value)
  }
  ...
  public func value(at position: Double, clamped: Bool) -> Double {
    let resolvedPosition =
      clamped
      ? min(range.upperBound, max(range.lowerBound, position))
      : position
    return value(at: resolvedPosition)
  }
}
